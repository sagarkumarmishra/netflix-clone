    ...
    <div>
      ...
      <main className="pt-24 mx-auto max-w-6xl px-5 pb-12 transition-all md:px-10">
        ...
        <div className="mt-6 grid grid-cols-1 gap-x-4 border px-4 py-4 md:grid-cols-4 md:border-x-0 md:border-t md:border-b-0 md:px-0 md:pb-0">
          <h4 className="text-lg text-[gray]">Plan Detais</h4>
          <div className="col-span-2 font-medium">
            {products.filter((product) => product.id === subscription?.product)[0]?.name}
          </div>
          <p className="cursor-pointer text-blue-500 hover:underline md:text-right" onClick={manageSubscription}>
            {isBillingLoading ? ('Loading...') : ('Change plan')}
          </p>
        </div>
